...
const FILTER_NODE_NAME: &str = "saq_virtual_sink";
...
const TARGET_SINK_KEY: &str = "saq.target.audio.sink";
            ...
            .global(move |object| {
                ...
                if is_default_metadata(object) {
                    match bind_default_metadata(&registry, object, &state_for_global) {
                        ...
                        Err(error) => {
                            eprintln!("Śaq could not observe PipeWire defaults: {error}")
                        }
                    }
                } else {
...
impl State {
    ...
    fn finish_shutdown(&mut self) {
        ...
        let Some(sink_id) = sink_id else {
            eprintln!("Śaq could not restore player links: the target output is unavailable");
            return;
        };
        ...
        for player_id in player_ids {
            for channel in [Channel::Left, Channel::Right] {
                ...
                match create_link(&self.core, output, input, false, true) {
                    ...
                    Err(error) => {
                        eprintln!("Śaq could not restore a direct player link: {error}")
                    }
                }
            }
        }
        if player_count > 0 && self.shutdown_links.is_empty() {
            eprintln!("Śaq could not restore any direct player links");
        }
    }
        ...
        for player_id in &player_ids {
            for channel in [Channel::Left, Channel::Right] {
                ...
                match create_link(&self.core, output, input, false, false) {
                    ...
                    Err(error) => {
                        eprintln!("Śaq could not connect an audio player: {error}")
                    }
                }
            }
        }
    ...
    fn route_output(&mut self) {
        ...
        for channel in [Channel::Left, Channel::Right] {
            ...
            match create_link(&self.core, output, input, true, false) {
                ...
                Err(error) => {
                    eprintln!("Śaq could not connect to the current audio output: {error}")
                }
            }
        }
    }
