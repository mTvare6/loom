...
pub fn run_audio_engine(shared_state: Arc<AudioState>) -> Result<(), pw::Error> {
    ...
    let capture_stream = pw::stream::StreamBox::new(&core, "saq_capture", capture_props)?;
    ...
    let playback_props = properties! {
        ...
        *pw::keys::NODE_NAME => "saq_hardware_out",
    };

    let playback_stream = pw::stream::StreamBox::new(&core, "saq_playback", playback_props)?;

    let mut engine = ŚaqEngine::new(48000.0);
